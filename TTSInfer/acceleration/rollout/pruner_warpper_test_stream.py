...
class CachePrunerWrapper:
    ...
    - Cache: 3cache (shared across layers) + 24cache (per-layer)
    ...
    def apply(
        ...
        if device.type == 'cuda' and not training:
            cache["worker_streams"] = [torch.cuda.Stream(device=device)]
            cache["decoder_stream"] = torch.cuda.Stream(device=device)
        ...
        def integrated_forward(self, sample, timestep, cond=None, **kwargs):
            ...
            worker_streams = cache_ctx.get("worker_streams")
            decoder_stream = cache_ctx.get("decoder_stream")
            ...
            if cur_step == 0 and cond is not None:
                ...
                if worker_streams is not None:
                    t0_submit = time.time()
                    with torch.cuda.stream(worker_streams[0]):
            ...
            # Step 1-99: Use precomputed results
                ...
                if cur_step == 1:
                    batch_ready_event = cache_ctx.get("batch_ready_event")
                    if batch_ready_event is not None:
                        batch_ready_event.synchronize()
                
                cache_ctx["timing"]["wait_pruner"] += time.time() - t0_wait
            ...
            # Timing stats output
            if cache_ctx["timing"]["total_steps"] % 100 == 0 and worker_streams is not None and sample.shape[0] == 1:
                timing = cache_ctx["timing"]
                total = timing["encoder_step0"] + timing["submit_pruner"] + timing["wait_pruner"] + timing["decoder"]
                print(f"\n=== CUDA Stream (after {timing['total_steps']} steps) ===")
                print(f"  Encoder:    {timing['encoder_step0']:.3f}s ({timing['encoder_step0']/total*100:.1f}%)")
                print(f"  Submit:     {timing['submit_pruner']:.3f}s ({timing['submit_pruner']/total*100:.1f}%)")
                print(f"  Wait:       {timing['wait_pruner']:.3f}s ({timing['wait_pruner']/total*100:.1f}%)")
                print(f"  Decoder:    {timing['decoder']:.3f}s ({timing['decoder']/total*100:.1f}%)")
                print(f"  Total:      {total:.3f}s ({total/timing['total_steps']:.4f}s/step)")
    ...
    def _batch_compute_pruner(policy: Any) -> None:
        ...
        num_steps = cache_ctx.get("num_steps", 100)
        worker_streams = cache_ctx["worker_streams"]
        encoder_buffer_batch = cache_ctx.get("encoder_buffer_batch")
        ...
        batch_ready_event = torch.cuda.Event()
        cache_ctx["batch_ready_event"] = batch_ready_event
        ...
        batch_ready_event.record(worker_streams[0])
    ...
    def _inject_layer_forward_rollout(layer: nn.Module, layer_name: str, policy: Any, training: bool, if_24cache: bool = False) -> None:
        ...
        cache_ctx = policy._cache
        # Extract layer_idx from layer_name, e.g. 'decoder.layers.0' -> 0
        match = re.search(r'\.(\d+)$', layer_name)
        layer_idx = int(match.group(1)) if match else 0
